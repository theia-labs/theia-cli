...
async function calculateAndExportScore({examId, token, environment}) {
    ...
    exporter.exportFile(process.env.EXPORT_PATH, exporter.EXPORT_TYPES.examScore, exporter.FORMATS.csv, fileContent, examId)
}

async function calculateAndExportScoreForMultipleExams({token, environment}) {
    const examScores = await calculateScoreForMultipleExams({token, environment})
    ...
    exporter.exportFile(process.env.EXPORT_PATH, exporter.EXPORT_TYPES.allExamsScore, exporter.FORMATS.csv, fileContent)
}
...
async function calculateScoreForMultipleExams({token, environment}) {
    const resourceFinder = new ResourceFinder(token, environment)

    /** @var {[Exam]} */
    const exams = await resourceFinder.fetchAllExams();

    const results = [];
    for (exam of exams) {
        results.push(await getExamResults(exam, resourceFinder))
    }
...
const getScoreByAssessmentSession = (assessmentSession) => {
    if (assessmentSession !== undefined && assessmentSession.hasOwnProperty('questionFoldersResults') && assessmentSession.questionFoldersResults !== null) {
        ...
        return {
            foldersScore,
            averageWithFolderWeight: totalWithFolderWeight.score / totalWithFolderWeight.weight * process.env.RATING_SCALE,
            averageWithoutFolderWeight: totalWithoutFolderWeight.score / totalWithoutFolderWeight.weight * process.env.RATING_SCALE
        }
    }
}
...
async function getParticipantName(participantId, participantList, resourceFinder) {
    ...
    return await resourceFinder.fetchResource(participant.user, false)
        .then(user => ({firstName: user.firstName ?? '', lastName: user.lastName ?? ''}))
        .catch(() => ({firstName: 'unknown user', lastName: 'unknown user', }))
}

module.exports = {calculateAndExportScore, calculateAndExportScoreForMultipleExams}
