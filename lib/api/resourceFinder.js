...
class ResourceFinder {
    ...
    async fetchSubscriptionGroupRules (groupRuleId) {
        return await this.fetchResource('/api/group_subscription_rules/' + groupRuleId);
    }

    async fetchGroupSubscriptionRules (groupIds, page = null) {
        let pa = (page) ? '&page=' + page : '';
        let route = '/api/group_subscription_rules?group[]=' + groupIds.replace(',', '&group[]=') + pa;
        return await this.fetchCollection(route);
    }

    async fetchGroup (groupId) {
        return await this.fetchResource('/api/groups/' + groupId);
    }
